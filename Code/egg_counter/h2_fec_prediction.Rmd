...
fm_stan <- map2stan(
  alist(
    ...
    mu <- barea * area + bimg_size * img_size,
    barea ~ dnorm(0, 5),
    bimg_size ~ dnorm(0, 1),
    sigma ~ dcauchy(0, 5)
  ),
...
```{r eval=FALSE}
area <- sqrt(seq(0, 1.25 * max(HC$area), length.out = 200))
...
ggplot() +
  geom_line(data = new_data, aes(x = area^2, y = lm_pred)) +
  geom_point(data = HC, aes(x = area, y = handcount))
```
...
suppressWarnings(
  M <- read_excel("../../Data/Processed/feclife_with-image-ids.xlsx") %>% 
    select(cameraid, handcounted, handcount, training_set,
           drop_from_lifespan, fID, treat)
)
