...
Round 2 of handcounts

```{r}
HC_2 <- read_excel("../../Data/Processed/handcount_new_bad_images.xlsx") %>% 
  rename(handcount = count) %>% 
  mutate(camera_id = paste0("IMG_", camera_id, ".JPG"))

h2_fec_areas <- read_csv("../../Data/Processed/area_summation_linear_h2_fecimages.csv") %>% 
  filter(lower_thresh == 46) %>% 
  filter(camera_id %in% HC_2$camera_id)

HC_2 <- left_join(HC_2, h2_fec_areas) %>% 
  drop_na() %>% 
  rename(area_linear = area) %>% 
  mutate(area_linear = as.integer(area_linear),
         handcount = as.numeric(handcount))

img_size_HC_2 <- read_csv("../../Data/Processed/image_dimensions_h2_fec_images.csv")

HC_2 <- left_join(HC_2, img_size_HC_2)

# Add to original handcounts
HC <- bind_rows(HC, HC_2) %>% 
  select(-lower_thresh)
```
