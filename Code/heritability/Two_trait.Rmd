...
early.fec <- data.frame("id" = character(length=length(all.ids)), "earlyegg"=numeric(length=length(all.ids)), stringsAsFactors = FALSE)
...
sum.dat <- early.dat %>% 
  group_by(Diet) %>% 
  summarise(life = mean(Median_Lifespan), fec = mean(earlyegg), lifese = sd(Median_Lifespan)/sqrt(length(Median_Lifespan)), fecse =sd(earlyegg)/sqrt(length(earlyegg)))
...
early.dat %>% 
  ggplot(aes(x = Median_Lifespan, y = earlyegg, color = Diet)) +
  geom_point() +
  geom_point(data=sum.dat, aes(life, fec, fill=Diet), size=5, pch=21, color='black') +
  geom_errorbar(data=sum.dat, aes(x=life, ymin=fec-fecse, ymax=fec+fecse)) +
  #geom_smooth(se = FALSE, method = "lm") +
  tx_color_map() +
  tx_fill_map()
