...
```{r MANOVA_Analysis}
load("../../Data/Processed/tri_model_prior1.Rda")
...
M <- as_tibble( M ) %>%  
  select(`HS vs. STD`, `LY vs. STD`, `HS vs. LY`) %>% 
  rename(`HS vs. C` = `HS vs. STD`) %>% 
  rename(`DR vs. C` = `LY vs. STD`) %>% 
  rename(`HS vs. DR` = `HS vs. LY`)
...
# ??? Heritability ????
HS <- re[, "treatHS:treatHS.sire"] / 
  (re[, "treatHS:treatHS.sire"] + re[, "units"])
median(HS)

LY <- re[, "treatLY:treatLY.sire"] / 
  (re[, "treatLY:treatLY.sire"] + re[, "units"])
median(LY)

STD <- re[, "treatSTD:treatSTD.sire"] / 
  (re[, "treatSTD:treatSTD.sire"] + re[, "units"])
median(STD)

HS_LY_STD <- cbind(HS, LY, STD) %>% as.data.frame()

HS_LY_STD %>% 
  gather(key = "Diet", value = "Heritability") %>% 
  ggplot(aes(Heritability, color = Diet)) +
