...
```{r, dens}
pdf("../../Figures/density_all-fec_data.pdf",width=6,height=4)
...
pdf("../../Figures/mean_family_fecundity.pdf",width=6,height=4)
par(mar=c(4.5,4.5,0.5,0.5))
...
```{r}
pdf("../../Figures/h2fec_reaction_norms.pdf",width=6,height=4)
par(mar=c(4.5,4.5,0.5,0.5))
ggplot(B, aes(x = diet, y = eggs, color = sireid)) +
  geom_point(size = 3) +
  geom_line(aes(group = sireid), alpha = 0.5, size = 1.5) +
  xlab("Diet") +
  ylab("Total eggs") +
...
lml <- ggplot(AA, aes(x = age, y = eggs_per_female, color = Diet)) +
...
AA$eggs_per_female_C <- AA$eggs_per_female - mean(AA$eggs_per_female, na.rm = TRUE)

# http://conjugateprior.org/2013/01/formulae-in-r-anova/
library(rstanarm)
library(lme4)
fm_lmer <- lmer(eggs_per_female_C ~ Diet + (1 + age | sireid),
                data = AA)
summary(fm_lmer)

fm_null <- stan_lmer(eggs_per_female_C ~ 1 + (1 + age | sireid),
                data = AA,
                cores = 4)
plot(fm_null, pars = c("(Intercept)", "DietDR", "DietHS"), plotfun = "trace")

fm_full <- stan_lmer(eggs_per_female_C ~ 0 + Diet + (1 + age | sireid),
                data = AA,
                cores = 4)
plot(fm_null, pars = c("(Intercept)", "DietDR", "DietHS"), plotfun = "trace")
```

# Layout with gridExtra 

```{r}
#library(gridExtra)
#grid.arrange(a,b,c,lml, ncol=2)
```
