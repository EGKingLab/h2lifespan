...
AA%>%
group_by(Diet)%>% 
summarise(Mean=mean(eggs_per_female), Max=max(eggs_per_female), Min=min(eggs_per_female), Median=median(eggs_per_female), Std=sd(eggs_per_female))
...
library(rstanarm)
library(lme4)
fm_lmer <- lmer(eggs_per_female_C ~ treat + age + treat:sireid + (1 | sireid),
                data = AA)
summary(fm_lmer)
...
fm_null <- stan_lmer(eggs_per_female_C ~ 1 + (1 + age | sireid),
                data = AA,
                cores = 4)
plot(fm_null, pars = c("(Intercept)"), plotfun = "trace")

fm_full <- stan_lmer(eggs_per_female_C ~ 0 + treat + treat:sireid + (1 + age | sireid),
                data = AA,
                cores = 4)
plot(fm_full, pars = c("treatLY", "treatHS", "treatSTD"), plotfun = "trace")
```
