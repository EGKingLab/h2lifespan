...
```{r}
AA %>%
     mutate(treat=replace(treat, treat=="LY", "DR")) %>%
  mutate(treat=replace(treat, treat=="STD", "C"))

AA <- rename(AA, Diet = treat)
AA
```
...
```{r}
#line plot
ll <- ggplot(data=AB.c, aes(x=age, y=eggs_per_female, color=Diet, group=id)) + #group-sire_trt
...
#HS <- subset(M, c(Diet == "HS", flipDate=="2016-06-07"))
HS_date <- select(J, camera_id, flipDate, fID, Diet, days)
HS_date$flipDate <- as.character(HS_date$flipDate)

HS_Jun7 <- HS_date[ which( HS_date$Diet == "HS" & HS_date$flipDate=="2016-06-07") , ]
HS_Jun14 <- HS_date[ which( HS_date$Diet == "HS" & HS_date$flipDate=="2016-06-14") , ]
HS_Jun21 <- HS_date[ which( HS_date$Diet == "HS" & HS_date$flipDate=="2016-06-21") , ]
...
HS_M_date <- select(M, camera_id, flipDate, fID, Diet, days, predicted_count_linear)
HS_M_date$flipDate <- as.character(HS_M_date$flipDate)

HS_M_Jun7 <- HS_M_date[ which( HS_M_date$Diet == "HS" & HS_M_date$flipDate=="2016-06-07") , ]
HS_M_Jun14 <- HS_M_date[ which( HS_M_date$Diet == "HS" & HS_M_date$flipDate=="2016-06-14") , ]
HS_M_Jun21 <- HS_M_date[ which( HS_M_date$Diet == "HS" & HS_M_date$flipDate=="2016-06-21") , ]
