...
k6 <- ggplot(data=h2fec_phenotype, aes(x=Diet, y=egg_total, color=Diet)) + #group-sire_trt
...
AA <- unite(AA, "sire_trt", sireid, treat, sep = "_", remove=FALSE)
...
#AA$wks <-  with(AA, ave(flipDate, flipDate, FUN = seq)) 
...
#AB$Time_Week<-1
#AB$Time_Week<-unlist(lapply(split(AB$Time_Week, AB$julian_date), cumsum)) 
...
A3 <- AB.c %>%
   select(Time_Week,treat,eggs_per_female)


#A3$Time_Week <- as.Date(AA$Time_Week)
A3$treat <- as.character(AA$treat)

A3 <- aggregate(.~Time_Week+treat, A3, mean)

k <- ggplot(A3, aes(x=Time_Week, y=eggs_per_female))+
  geom_line(aes(color=treat)) +
  xlab("Time (week)") + ylab("24 Hr Average Fecundity")
k

k1 <- ggplot(A3, aes(x=Time_Week, y=eggs_per_female, color=treat))+
...
k2 <- ggplot(A3, aes(x=Time_Week, y=eggs_per_female, color=treat))+
  geom_point(aes(shape=treat)) +  geom_smooth(method="loess") +
  xlab("Time (week)") + ylab("24 Hr Average Fecundity")
k2 + facet_grid(. ~ treat)#facet_grid(treat~ .) 


k3 <- ggplot(A3, aes(x=Time_Week, y=eggs_per_female, color=treat))+
  geom_point(aes(shape=treat)) +  geom_smooth(method="loess")+
  xlab("Time (week)") + ylab("24 Hr Average Fecundity")
k3 + facet_grid(treat~ .) 

#volcano
k4 <- ggplot(data=A3, aes(x = eggs_per_female))
k4 + stat_density(aes(ymax = ..density..,  ymin = -..density.., 
                       fill = treat, color = treat), 
                   geom = "ribbon", position = "identity") +
  xlab("Time (week)") + ylab("24 Hr Average Fecundity")+
  
  facet_grid(. ~ treat) + coord_flip() + xlab("Time (weeks)") 
...
```{r}
#line plot
ll <- ggplot(data=AB.c, aes(x=Time_Week, y=eggs_per_female, color=treat, group=id)) + #group-sire_trt
  #geom_line() #+ ggtitle("blah-blah")+
...
```{r}
k5 <- AB.c 
k5 <- subset(AB.c, AB.c$eggs_per_female < 101)

ggplot(data=k5, aes(x=Time_Week, y=eggs_per_female, color=treat, group=id)) + #group-sire_trt
  geom_jitter(position = position_jitter(width = 0.3), alpha = 0.5) +
  xlab("Time (week)") + ylab("24 Hr Average Fecundity")
k5
```
...
# the tidy way
AA %>%
    group_by(flipDate, treat) %>%
    summarize(eggs = sum(eggs_per_female))
```
...
library(readxl)
suppressWarnings(
  J <- read_excel("../../Data/Processed/feclife_with-image-ids.xlsx")
)

#HS <- subset(M, c(treat == "HS", flipDate=="2016-06-07"))
HS_date <- select(J, camera_id, flipDate, fID, treat, days)
HS_date$flipDate <- as.character(HS_date$flipDate)

HS_Jun7 <- HS_date[ which( HS_date$treat == "HS" & HS_date$flipDate=="2016-06-07") , ]
HS_Jun14 <- HS_date[ which( HS_date$treat == "HS" & HS_date$flipDate=="2016-06-14") , ]
HS_Jun21 <- HS_date[ which( HS_date$treat == "HS" & HS_date$flipDate=="2016-06-21") , ]
...
HS_M_date <- select(M, camera_id, flipDate, fID, treat, days, predicted_count_linear)
HS_M_date$flipDate <- as.character(HS_M_date$flipDate)

HS_M_Jun7 <- HS_M_date[ which( HS_M_date$treat == "HS" & HS_M_date$flipDate=="2016-06-07") , ]
HS_M_Jun14 <- HS_M_date[ which( HS_M_date$treat == "HS" & HS_M_date$flipDate=="2016-06-14") , ]
HS_M_Jun21 <- HS_M_date[ which( HS_M_date$treat == "HS" & HS_M_date$flipDate=="2016-06-21") , ]
