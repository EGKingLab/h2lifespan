...
```{r}
# sepatate columns
  colum_hs <- select(h2fec_phenotype, sireid,hs)
  colum_c <- select(mlife, sireid,c)
  colum_dr <- select(mlife, sireid, dr)

  h2_all <- rbind(C, HS, DR) 
```


```{r}
#combine data frames
mlife <- merge(hs_med, merge(c_med, dr_med, by="sireid", all.x=TRUE, all.y=TRUE), by = "sireid", all.x = TRUE, all.y = TRUE)
mlife[, 3] <- NULL
mlife[, 4] <- NULL
mlife[, 5] <- NULL
colnames(mlife) <- c("sireid", "hs","c","dr")
```


```{r}
#pairs(~hs+c+dr,data=mlife)
pdf("/Users/ngomae/Documents/H2_Analysis/h2lifespan/Evo17_Portland/lifecorrel_pearson.pdf",width=6,height=4)

#sapply(mlife, is.numeric)   #which columns are numeric
#p <- mlife[, sapply(mlife, is.numeric)] #grab numeric columns

ggcorr(mlife, palette = "RdYlGn", name = "pearson", #use 'rho' for ranks
       label = TRUE, label_color = "black", size=12)
dev.off()
```
