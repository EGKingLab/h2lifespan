...
```{r}
h2.sireid <- survfit(Surv(NewAge, status == 2) ~ sireid,
                     data = h2life,
                     type = "kaplan-meier",
                     error = "greenwood")

surv_2 <- ggsurvplot_facet(h2.sireid, h2life, 
                           facet.by = "treat",
                           size = 0.6,
                           risk.table = FALSE, 
                           pval = FALSE, 
                           conf.int = FALSE,
                           panel.labs = list(treat = c("C", "DR", "HS")),
                           surv.median.line = "none") +
  xlab("Time (days)")
ggpar(surv_2, legend = "none",
...
# Kaplan-Meier plot for sire families singly
...
ggplot(mlife1, aes(x = mlife1$diet, y = mlife1$age, color = sireid)) +
  geom_point(size = 3) +
