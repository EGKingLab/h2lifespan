...
```{r}
# by treatment groups
h2.treat <- survfit(Surv(NewAge, status == 2) ~ treat,
                    conf.type = "log", 
                    conf.int = 0.95,
                    type = "kaplan-meier",
                    error = "greenwood",
...
# survival
s_plot <- ggsurvplot(h2.treat, data = h2life, 
           conf.int = TRUE,
           palette = pal,
           fun = "pct", 
           surv.median.line = "hv",
...
KM_plot <- last_plot()
ggsave(last_plot(), file = "../../Figures/K-M_treat_summarized_survminer.pdf",
       width = 4, height = 4)
...
```{r}
h2.treat.coxph <- coxph(Surv(NewAge, status == 2) ~ treat,
                      data = h2life)
h2.treat.coxme.sire <- coxme(
  Surv(NewAge, status == 2) ~ treat + (1 | sireid),
  data = h2life)
h2.treat.coxme.sire.dam <- coxme(
  Surv(NewAge, status == 2) ~ treat + (1 | sireid) + (1 | sireid/damid),
  data = h2life)
anova(h2.treat.coxph, h2.treat.coxme.sire, h2.treat.coxme.sire.dam)

summary(h2.treat.coxme.sire.dam)
anova(h2.treat.coxme.sire.dam)
```
...
```{r testdiff}
test_all <- survdiff(Surv(NewAge, status == 2) ~ treat,
                     data = h2life, rho = 0) #log-rank (Mantel-Haenszel) test
test_all1 <- survdiff(Surv(NewAge, status == 2) ~ treat,
                      data = h2life, rho = 1) # Wilcoxon (Peto & Peto test)
test_all
test_all1

quantile(h2.treat,  probs = c(0.1, 0.5, 0.9))
```
...
#colourCount = length(unique(h2life$sireid))
#getPalette = colorRampPalette(brewer.pal(12, "Set3"))

#labels <- c(`treat: C` = "C", `treat: DR` = "DR", `treat: HS` = HS)
...
kk <- ggsurvplot_facet(h2.sireid, h2life, 
                 ...
                 #legend ="none",
                 surv.median.line = "hv")+
                 #colorRampPalette(brewer.pal(12, "Paired")))
                 #palette = "jco")
   xlab("Time (days)") +
  scale_y_continuous(expand = c(0, 0))
...
ggsave(last_plot(), file = "../../Figures/K-M_treat_sire_survminer.pdf",
       width = 8, height = 4)
```
...
```{r}
h2.sire <- survfit(Surv(NewAge, status == 2) ~ treat, data = h2life)
...
ggsave(file = "../../Figures/K-M_treat_single-sires_survminer.pdf",
       width = 8, height = 6)
```
...
DR <- subset(h2rn_filtered, treat == "DR")
...
C <- subset(h2rn_filtered, treat == "C")
...
```{r}
km.treat <- survfit(Surv(NewAge, status == 2) ~ id,
                    conf.type = "log", 
                    conf.int = 0.95,
                    type = "kaplan-meier",
                    error = "greenwood",
