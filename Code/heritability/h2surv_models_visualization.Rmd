...
```{r cache=TRUE}
cox1 <- coxph(Surv(NewAge, status == 2) ~ 1,
                      data = h2life)
cox2 <- coxph(Surv(NewAge, status == 2) ~ treat,
                      data = h2life)
cox3 <- coxme(
  Surv(NewAge, status == 2) ~ treat + (1 | sireid),
  data = h2life)
cox4 <- coxme(
  Surv(NewAge, status == 2) ~ treat + (1 | sireid/damid),
  data = h2life)
cox5 <- coxme(
...
bm1 <- brm(NewAge | cens(censored) ~ 1,
           ...
           family = weibull(),
           inits = "0")
bm2 <- brm(NewAge | cens(censored) ~ treat,
           data = h2life_2,
           family = weibull(),
           inits = "0")
...
compare_models(loos = list(loo_bm1, loo_bm2))
loo_model_weights(list(loo_bm1, loo_bm2))
