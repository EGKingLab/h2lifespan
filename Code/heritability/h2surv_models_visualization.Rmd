...
```{r cox1_cox_5, cache=TRUE}
cox1 <- coxph(Surv(NewAge, status == 2) ~ 1,
                      data = h2life)
cox2 <- coxph(Surv(NewAge, status == 2) ~ diet,
                      data = h2life)
cox3 <- coxme(
  Surv(NewAge, status == 2) ~ diet + (1 | sireid),
  data = h2life)
cox4 <- coxme(
  Surv(NewAge, status == 2) ~ diet + (1 | sireid/damid),
  data = h2life)
cox5 <- coxme(
  Surv(NewAge, status == 2) ~ diet + (1 | sireid) + (1 | sireid/damid),
  data = h2life)
```
