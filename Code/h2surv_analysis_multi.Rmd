...
data_frame(x = seq(1, 10, length = 100),
           y = dinvgamma(x, shape = nu / 2, scale = nu * V / 2)) %>% 
  ggplot(aes(x, y)) +
  geom_line() +
...
fm_rest <- stan_lmer(
  ...
  iter = 1e4,
  warmup = 1e3)
save(fm_full, fm_rest, file = "rstanarm_mods.Rda")
```
...
# Wilson et al revised
# prior <- list(G = list(G1 = list(V = diag(3), n = 1.002)),
#               R = list(V = diag(3), n = 1.002))

# See de Villemereuil
# inverse-Gamma(0.001, 0.001) on variances; and the (marginal) prior
# distribution of correlation follows to some extent Beta(0.001, 0.001
...
# Genetic correlation
# corr.gen <- fm$VCV[ , 'phen1:phen2.animal'] / 
#   sqrt(fm$VCV[ , 'phen1:phen1.animal'] * fm$VCV[ , 'phen2:phen2.animal'])
# mean(corr.gen)
